...
;(function ($, window, document, undefined) {
  ...
  var pluginName = 'bootstrapDualListbox',
    defaults = {
      ...
      infoText: '当前未选择({0})',                                                        // text when all options are visible / false for no info text
      infoTexts: '当前已选择({0})',                                                        // text when all options are visible / false for no info text
      infoTextFiltered: '<span class="label label-warning">Filtered</span> {0} from {1}', // when not all of the options are visible due to the filter
      infoTextEmpty: '未选择',                                                        // when there are no options present in the list
  ...
  function refreshInfo(dualListbox) {
    ...
    if (all2 === 0) {
      ...
      content = formatString(dualListbox.settings.infoTexts, [visible2, all2]);
    } else {
