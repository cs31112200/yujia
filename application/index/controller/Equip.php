 ...
 */    
    public function addEditOclock(){
        $member_id =$this->verifyUser();
        $this->__checkParam('area_id,is_open,open_time',$this->input);
        extract($this->input);
        //验证area_id
...
/*query
 * @param type 115,230,31xiaoshi,4月5
 */    
    public function query_log(){
        ...
        if($type>=3 && !isset($the_time)){
            $this->returnJson(0,'请选择正确的时间');
        }
        $the_time = !isset($the_time)?"":$the_time;
